...
export default function App() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (!inputs.triggered_at_utc) throw new Error("Triggered At (UTC) is required. Format: YYYY-MM-DD HH:MM:SS");

      // pull candles
      const { mark, last } = await getTriggerMinuteCandles(inputs.symbol, inputs.triggered_at_utc);
      const tMinute = new Date(msMinuteStartUTC(inputs.triggered_at_utc)).toISOString().slice(0,16).replace("T"," ");
